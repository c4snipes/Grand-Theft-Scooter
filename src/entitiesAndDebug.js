...
export function initChunking(scene, world) {
  ...
  return {
    ...
    setChunkingConfig({ size, radius, enabled }) {
      ...
      if (typeof radius === 'number' && isFinite(radius) && radius >= 1)
        chunkRadius = Math.floor(radius);
...
export function mallSearch({ interactables, mallBounds }) {
  ...
  function enforceCentralClearance(candidate, clearance) {
    ...
    if (planarDistance < minimumRadius) {
      ...
      const safeAngle =
        planarDistance < 1e-4 ? Math.random() * Math.PI * 2 : Math.atan2(candidate.z, candidate.x);
...
export const mallSpawning = {
  registerInteractable({ opts, scene, world, materials, interactables, dynamicActors, chunking }) {
    const {
      mesh,
      body,
      label,
    ...
    if (chunking?.chunkedStreamingEnabled?.()) {
      const key =
        chunkKey ?? `${Math.floor(body.position.x / 48)},${Math.floor(body.position.z / 48)}`;
      record.chunkKey = key;
      mountRecordToScene({
        record,
        scene,
        chunkKey: key,
        ensureChunk: chunking.ensureChunk,
      });
    } else {
      scene.add(mesh);
    }
  ...
  createHitHandlers({ scene, world, interactables, dynamicActors }) {
    ...
    function handleHit(record, hitterBody) {
      ...
      if (!record.type) {
        warnOnce('mall:handleHit:missingType', '[mall.handleHit] Record is missing a type.', {
...
export function createMall({ world, scene, assets = {}, materials = {} } = {}) {
  ...
  const useMallAsset =
    Boolean(assets.mallScene) && !Boolean(assets.mallScene?.userData?.isProceduralMall);
  const kioskFactory =
    typeof assets.makeKioskInstance === 'function' ? assets.makeKioskInstance.bind(assets) : null;
  const columnFactory =
    typeof assets.makeColumnInstance === 'function' ? assets.makeColumnInstance.bind(assets) : null;
  const bannerFactory =
    typeof assets.makeBannerInstance === 'function' ? assets.makeBannerInstance.bind(assets) : null;
...
export function buildFallbackScooterMesh() {
  ...
  const primaryPaint = new MeshStandardMaterial({
    color: '#f0567d',
    metalness: 0.3,
    roughness: 0.55,
  });
  const accentPaint = new MeshStandardMaterial({
    color: '#292d36',
    roughness: 0.9,
  });
  ...
  wheelsMeta.frontRadius =
    computeWheelRadius(wheelsMeta.frontWheel) ?? wheelsMeta.frontRadius ?? 0.18;
...
function buildScooterMeshFromAssets(assets = {}) {
  ...
  const scooterRoot =
    assets.scooterScene && typeof assets.scooterScene.clone === 'function'
      ? assets.scooterScene.clone(true)
      : null;
  if (!scooterRoot) {
    console.warn('[assets] Scooter scene missing clone(); using fallback mesh.');
    return { group: buildFallbackScooterMesh(), mixers: [] };
  }
  ...
  wheelsMetaFromAsset.frontRadius =
    computeWheelRadius(wheelsMetaFromAsset.frontWheel) ?? wheelsMetaFromAsset.frontRadius ?? null;
  wheelsMetaFromAsset.rearRadius =
    computeWheelRadius(wheelsMetaFromAsset.rearWheel) ?? wheelsMetaFromAsset.rearRadius ?? null;
